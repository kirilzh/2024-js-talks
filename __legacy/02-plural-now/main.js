...
function tickManager() {
  ...
  return {
    ...
    move: function (value) {
      ...
      if (tick) {
        const initialRect = tick.getBoundingClientRect();
        consumedBlock.appendChild(tick);
        const finalRect = tick.getBoundingClientRect();

        const deltaX = initialRect.left - finalRect.left;
        const deltaY = initialRect.top - finalRect.top;
        tick.style.transform = `translate(${deltaX}px, ${deltaY}px)`;

        tick.offsetHeight; // Trigger reflow

        tick.style.transition = 'transform 0.5s ease';
        tick.style.transform = 'translate(0, 0)';

        tick.addEventListener('transitionend', () => {
